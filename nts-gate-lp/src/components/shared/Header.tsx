...
export default function Header() {
  ...
  return (
    ...
    >
      {isSubsidies ? (
        <>
          {/* スマホ: ロゴ + CTA + メニュー。sm以上は display: contents で既存のPC配置を維持 */}
          <div className="flex items-center justify-between gap-2 sm:contents">
            ...
            <div className="flex shrink-0 items-center gap-2 sm:hidden">
              ...
              <button
                ...
                aria-controls="mobile-subsidies-nav-menu"
          ...
          <div
            id="mobile-subsidies-nav-menu"
            className={`${
              mobileMenuOpen ? "flex" : "hidden"
            } -mx-4 flex-col border-t border-[rgba(26,76,142,0.1)] bg-white/95 px-4 pb-1 pt-2.5 sm:mx-0 sm:flex sm:flex-1 sm:flex-row sm:items-center sm:border-0 sm:bg-transparent sm:px-0 sm:pb-0 sm:pt-0`}
          >
            {/* PC: 従来の横1列ナビ。モバイル: メニュー内の縦ナビ */}
            <nav
              className="flex min-w-0 flex-col gap-1 sm:flex-1 sm:flex-row sm:items-center sm:justify-end sm:gap-x-3 sm:overflow-hidden sm:ml-2 lg:ml-6 lg:gap-x-5"
              aria-label="補助金プラットフォーム"
            >
              {SUBSIDY_NAV_ITEMS.map(({ label, href, exact }) => {
                ...
                return (
                  <Link
                    key={href}
                    href={href}
                    className={`shrink-0 whitespace-nowrap rounded-lg px-2 py-2 text-sm transition-colors hover:bg-[rgba(26,76,142,0.05)] sm:rounded-none sm:px-0 sm:py-0 sm:text-xs sm:hover:bg-transparent lg:text-sm ${
                      isActive
              ...
              <Link
                ...
                className="shrink-0 whitespace-nowrap rounded-lg px-2 py-2 text-sm font-medium text-[var(--text-secondary)] transition-colors hover:bg-[rgba(26,76,142,0.05)] hover:text-[var(--accent-navy)] sm:rounded-none sm:px-0 sm:py-0 sm:text-xs sm:hover:bg-transparent lg:text-sm"
              >
                提携先の方へ
              </Link>
              <Link
